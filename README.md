...
10. The header **Settings** (gear) opens a **dropdown** with **Color palette** (Midnight Gold, Arcade Night, Pine Screen, Daylight), **Export to PDF**, **Export to CSV**, and **Clear Session**. Palettes apply with CSS variables and are stored in this browser. PDF uses the browser print dialog (save as PDF). CSV downloads the current catalog’s log. Clear Session asks for confirmation, then wipes both Movies and Games logs.
