...
13. Ranx, Tourney, WTF, the session log, and Wikipedia search show a **portrait** poster only from a Wikipedia **film** (or video game, or album) page — not people, books, or same-name articles. Album covers prefer [Cover Art Archive](https://coverartarchive.org/) for MusicBrainz release groups. The caption credits Wikipedia/Wikimedia or Cover Art Archive. A loaded movie/game poster is a link to that title’s **IMDb** page (new tab). Album posters link to **MusicBrainz**. If a poster is not loaded within **10 seconds**, the same 2:3 frame links to IMDb or MusicBrainz instead. Overlay star/bookmark and Select stay usable. The current stack’s posters are preloaded so the next card is less likely to wait.
