...
5. **Tourney** deals two titles. **Select** one, then score the Contender 1–10 or **Skip scoring**. The unselected title goes to **Discard**. Check **Remember this setting** to skip scoring on later Contenders (turn it back off from the next matchup). **Reshuffle** under the pair skips both cards with no winner and deals a new matchup; those titles stay out of the next deals.
...
13. Rank, Tourney, WTF, the session log, and Wikipedia search show a poster when Wikipedia or Wikimedia Commons has one. The caption credits the source with a link. IMDb ids from the movie catalog can resolve a Commons still via Wikidata (no IMDb API key). Missing images stay as an empty frame.
