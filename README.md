...
2. Choose **Tourney** or **Ranx** — equal choices. Wikipedia search stays hidden until a mode is selected.
...
10. The header **Settings** (gear) opens a **dropdown** with **Color Palette**, **Export**, and **Clear Session**. **Color Palette** opens a **modal** on phone and desktop (schemes are not listed inline in the menu) with Midnight Gold, Arcade Night, Pine Screen, Daylight, plus the extra palettes (Void Signal, Cabin Moss, Clear Sky, Harbor Gold, Pale Harbor, Orchid Amber, Neon Ticket, Blush Note, Graphite, Steel Marquee, Wine Mint, Olive Print, Warm Slate, Navy Brass). Palettes apply with CSS variables and are stored in this browser. **Export** opens a modal with **Export to PDF** (browser print / save as PDF), **Export to CSV** (this catalog’s log), and **Letterboxd CSV** (UTF-8 Letterboxd import for **movies** only: ratings as Rating10, reviews, title/year, Directors when known, imdbID when known, WatchedDate when you entered a watch date; Watch/Want rows have no rating). Clear Session asks for confirmation, then wipes Movies, Games, and Music logs.
